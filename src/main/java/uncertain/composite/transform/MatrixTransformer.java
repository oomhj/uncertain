 ...
 * 
 */
public class MatrixTransformer implements CompositeTransformer, Serializable{
    ...
    public MatrixTransformer(
            Object 		rowKeyField, 
            String  	columnFieldStr,
            String 		colValueFieldStr,
    ...
    public MatrixTransformer(CompositeMap params){
        this();
        init(params);
    }
    
    void assertParamNotNull(String name, Object v){
        if(v==null) throw new ConfigurationError("[MatrixTransformer] required parameter "+name+" is not set");
    }
    ...
    public CompositeMap transform( CompositeMap source){
        ...
        while(it.hasNext()){
            ...
            CompositeMap target_map = (CompositeMap)grouped_rows.get(row_value); 
            if(target_map==null){                
            ...
            // create column
            CompositeMap col_config =(CompositeMap)grouped_cols.get(col_field); 
            if((col_config)!=null){
            }else{
            ...
            if(new_row)
	            for(int i=0; i<columnFields.length; i++){
	                if(sharedFieldSet!=null)
	                    if(sharedFieldSet.contains(columnFields[i])) continue;
            ...
            if(fieldValueOnly){
                ...
                if(colValueFields!=null)
	                for(int i=0; i<colValueFields.length; i++)
	                    col_config.put(colValueFields[i],item.get(colValueFields[i]));
                if(sharedFields!=null)
                    for(int i=0; i<sharedFields.length; i++)
                        col_config.put(sharedFields[i],target_map.get(sharedFields[i]));
        ...
        if( createMetaData ){
            Collections.sort(transformedColumns, new CompositeComparator(colKeyField.toString()));
    ...
    /**
     * @param fieldValueOnly The fieldValueOnly to set.
     */
